...
const Encounter4 = {
    SequenceDissection: function () {
        ...
        while (true) {
            ...
            steps.push({
                type: 'instruction',
                text: `${Localization.dictionary.agarrar} ${this._toIcon(sequence.componentA)} 
                    ${Localization.dictionary.from} ${Localization.dictionary[sequence.positionA]} 
                    ${Localization.dictionary.ponerlo} ${Localization.dictionary[sequence.positionB]}.`
            });

            steps.push({
                type: 'instruction',
                text: `${Localization.dictionary.agarrar} ${this._toIcon(sequence.componentB)} 
                    ${Localization.dictionary.from} ${Localization.dictionary[sequence.positionB]} 
                    ${Localization.dictionary.ponerlo} ${Localization.dictionary[sequence.positionA]}.`
            });
        ...
        if (steps != null) {
            ...
            for (let i = 0, step = 1; i < steps.length; i++) {
                switch (steps[i].type) {
                    ...
                    case 'info':
                        $('#steps').append(`<li>${steps[i].text}</li>`);
